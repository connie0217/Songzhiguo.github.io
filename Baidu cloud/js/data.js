...
/*    //渲染数据
    ...
    function creaSure() {
        ...
        for (var i = 0;i<data.length;i++){
            //当前Pid下
            if (data[i].pId == nowPid){
                //当前pid下所有数据存入数组identicalPid
                identicalPid.push(data[i]);
            }
        }
        //只要有一个等于createName
        if (identicalPid.some(function (a) {
                return a.name == createName;
            })) {
            //新建时box.value没变
            if (box.value == createName){
                for (var i =0;i<identicalPid.length;i++){
                    //文件名前五位等于新建文件夹
                    if (identicalPid[i].name.substr(0, box.value.length) == createName){
                        //文件名长度为8
                        if (identicalPid[i].name.length == createName.length +3){
                            //文件名第六位为(
                            if (identicalPid[i].name.substr(box.value.length, 1) == '(') {
                                //文件名第七位为数字
                                if (typeof Number(identicalPid[i].name.substr(box.value.length + 1, 1)) == 'number') {
                                    //文件名第八位为）
                                    if (identicalPid[i].name.substr(box.value.length + 2, 1) == ')') {
                                        //把文件名中的第七位数字存放postfix中
                                        postfix.push(identicalPid[i].name.substr(box.value.length + 1, 1));
                                        //数组从小到大排列
                                        postfix.sort();
                                    }
                                }
                ...
                if (postfix[0] == 2){
                    ...
                    for (var i = 0;i<postfix.length;i++){
                        if (Number(postfix[i]*1+1) != Number(postfix[i+1])){
                            arrI.push(i);
                            fileNames[0].innerHTML = createName + '(' + (postfix[arrI[0]]*1 +1) + ')';
                ...
                }else {//数组中的第一位不等2时，新建文件名直接为新建文件夹(2)
                    fileNames[0].innerHTML = createName + '(2)';
                    tipText.innerHTML = '新建成功';
                    // 提示框
                    tip();
                    //弹窗消失
            ...
            }else { //新建时box.value改变时，新建文件名为改变后的box.value
                if (identicalPid.some(function (a) {
                        return a.name == box.value;
                    })) {
                    alert('此目标已包含名为'+box.value+'的文件夹');
                    box.select();//默认选中input框中的文字
                }else {
        ...
        for (var i = 0; i < checkboxs.length; i++) {
            if (checkboxs[i].className == 'checkbox-lick') {
                ...
                if (identicalPid.some(function (a) {
                    ...
                    for (var j =0;j<identicalPid.length;j++){
                        //文件名前五位等于新建文件夹
                        if (identicalPid[j].name.substr(0, box.value.length) == box.value){
                            //文件名长度为8
                            if (identicalPid[j].name.length == box.value.length +3){
                                //文件名第六位为(
                                if (identicalPid[j].name.substr(box.value.length, 1) == '(') {
                                    //文件名第七位为数字
                                    if (typeof Number(identicalPid[j].name.substr(box.value.length + 1, 1)) == 'number') {
                                        //文件名第八位为）
                                        if (identicalPid[j].name.substr(box.value.length + 2, 1) == ')') {
                                            //把文件名中的第七位数字存放postfix中
                                            postfix.push(identicalPid[j].name.substr(box.value.length + 1, 1));
                                            //数组从小到大排列
                                            postfix.sort();
                                        }
                                    }
                    ...
                    if (postfix[0] == 2){
                        ...
                        for (var j = 0;j<postfix.length;j++){
                            if (Number(postfix[j]*1+1) != Number(postfix[j+1])){
                                arrI.push(j);
                                fileNames[i].innerHTML = box.value + '(' + (postfix[arrI[0]]*1 +1) + ')';
                                data[i].name = box.value + '(' + (postfix[arrI[0]]*1 +1) + ')';//改变数据中对应的name
                            }
                        }
                    }else {//数组中的第一位不等2时，新建文件名直接为新建文件夹(2)
                        fileNames[i].innerHTML = box.value + '(2)';
                        data[i].name = box.value + '(2)';//改变数据中对应的name
                    }
                }else {//没有createName
                    fileNames[i].innerHTML = box.value;
                    data[i].name = box.value;//改变数据中对应的name
                }
...
/*
    总结：
        1、利用hash做面包屑导
        2、新建重命名是我花费时间最多的功能，之前是思路的问题，写了好几个版本都没解决bug,转变思想后很快就解决了
*/
